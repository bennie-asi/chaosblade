...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --version)
            ...
            # Interpolated into TAG, the download URL and INSTALL_DIR — which
            # line 328 rm -rf's. A value with a slash currently fails earlier at
            # mktemp, but that is incidental ordering, not a guarantee.
            if [[ ! "$2" =~ ^[0-9]+(\.[0-9]+)*$ ]]; then
...
# Keep the previous version until the new one is in place. "rm -rf" followed by
# "mv" was destructive in the gap between them: anything that failed there (disk
# full, permissions, the old binary still mapped) left NEITHER version installed
# and no binary to re-run an installer with. Move the old directory aside, and
# only delete it once the new one has landed.
DISPLACED=""
if [[ -d "${INSTALL_DIR}" ]]; then
    DISPLACED="${INSTALL_DIR}.replaced-$$"
    rm -rf "${DISPLACED}"
    if ! mv "${INSTALL_DIR}" "${DISPLACED}"; then
        err "Cannot move aside existing install at ${INSTALL_DIR}"
    fi
fi

restore_displaced() {
    # Put the previous version back, then fail loudly.
    rm -rf "${INSTALL_DIR}"
    if [[ -n "${DISPLACED}" ]] && [[ -d "${DISPLACED}" ]]; then
        mv "${DISPLACED}" "${INSTALL_DIR}" || true
    fi
}

if ! mv "${TMP_DIR}" "${INSTALL_DIR}" 2>/dev/null; then
    # Cross-filesystem move: fall back to copy + remove
    if ! mkdir -p "${INSTALL_DIR}" || ! cp -r "${TMP_DIR}/." "${INSTALL_DIR}/"; then
        restore_displaced
