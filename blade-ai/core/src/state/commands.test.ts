...
describe("formatReviewCard", () => {
  ...
  it("renders the experiment_uid line when the envelope carries it", () => {
    // get_metric carries ``experiment_uid`` top-level (task_store.py,
    // renamed off ``blade_uid`` in phase-9). The card read the stale
    // key and the UID line was silently empty since phase-9; phase-14
    // aligned the read — this anchor pins the field contract.
    const card = formatReviewCard("task-1", {
      status: "success",
      experiment_uid: "e8f1a2b3c4d5e6f7",
    });
    expect(card).toContain("e8f1a2b3c4d5e6f7");
  });
