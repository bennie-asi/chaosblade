...
class TestClassifierFactsDivergences:
    ...
    def test_flagged_shell_peel_tightens_to_escape(self):
        ...
        assert _scope("exec mypod -- sh -x -c 'chroot /host id'") == classifier.SCOPE_ESCAPE
    ...
    def test_ansi_c_escape_head_recognised(self):
        ...
        assert _scope("exec mypod -- $'\\x63hroot' /host id") == classifier.SCOPE_READONLY

    def test_usr_bin_sh_basename_peeled(self):
        # Legacy's peel table missed ``/usr/bin/sh``.
        assert _scope("exec mypod -- /usr/bin/sh -c 'chroot /host id'") == classifier.SCOPE_READONLY

    # --- no-drift pins ---
    def test_plain_readonly_unchanged(self):
        assert _scope("exec mypod -- df -h") == classifier.SCOPE_READONLY

    def test_mutating_escape_still_escape(self):
        v = "exec mypod -- sh -c 'chroot /host iptables -A INPUT'"
