...
3. 注入网络包损坏故障，按上一步结论二选一。
   ...
   **先武装定时自删，再注入规则**（后台进程与目标容器同 netns，到期自动移除规则）：
   ...
   # exec 载荷（sh -c）的死参数，注入静默丢失。武装命令必须重定向后台化，
   # 否则 exec 挂住
...
**注入验证**：
1. 在目标 Pod 内查看网络重传统计（管道必须包在 sh -c 载荷内——命令是 argv 直传无 shell，
...
2. 在目标 Pod 内验证请求间歇性失败（计数用 while 自增而非 `$(seq)`——busybox 无 seq
   applet，`$(seq 1 N)` 展开为空会使探测循环空转零输出）：
   ```bash
