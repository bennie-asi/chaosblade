...
2. Pod Event 显示镜像拉取失败，错误信息包含镜像不存在类错误：docker 通道为
   ...
   `failed to resolve reference ... [404 Not Found]`（resolve 阶段直接 404，无 manifest 字样，实测 containerd 集群）
...
3. **武装定时自恢复**（恢复命令幂等：定时器到期自动还原为主，Agent 在演练结束时主动执行
   ...
   kubectl exec <载体Pod> -n <载体命名空间> -- sh -c 'echo <restore-b64> | base64 -d > /tmp/blade-restore-badimage.sh; ( sleep <duration>; sh /tmp/blade-restore-badimage.sh ) >/dev/null 2>&1 & echo armed'
   ```
...
3. 执行 `kubectl describe pod <pod-name>`，确认 Events 包含镜像不存在相关信息：docker 通道为
   ...
   （判读时不要按 manifest 字样硬匹配，两种通道措辞不同，实测 containerd 集群报 404）
